...
class LeafAreaGUI:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.root.geometry("1200x900")  # Initial size
    ...
    def create_main_frames(self) -> None:
        ...
        # Bottom Frame: Log Output
        self.bottom_frame = tk.Frame(self.scrollable_frame, height=500, bg="#f0f0f0")  # Increased height
    ...
    def create_controls(self, parent: tk.Frame) -> None:
        ...
        # Configure grid with 3 columns: 0 (main), 1 (entries), 2 (descriptions)
        control_frame.grid_columnconfigure(0, weight=1)  # Main settings
        control_frame.grid_columnconfigure(1, weight=0)  # Entries
        control_frame.grid_columnconfigure(2, weight=0)  # Descriptions
        ...
        self.dir_label.grid(row=1, column=0, columnspan=3, pady=5, sticky='w')  # Spanning all columns
        ...
        cropping_frame.grid(row=2, column=0, columnspan=3, pady=5, sticky='we')  # Spanning all columns

        # Image Cropping Section
        cropping_settings = [
            ("Crop Left (%)", "crop_left", "Percentage to crop from the left side of the image."),
            ("Crop Right (%)", "crop_right", "Percentage to crop from the right side of the image."),
            ("Crop Top (%)", "crop_top", "Percentage to crop from the top of the image."),
            ("Crop Bottom (%)", "crop_bottom", "Percentage to crop from the bottom of the image.")
        ]

        for idx, (label_text, key, description) in enumerate(cropping_settings):
            ...
            desc_label = tk.Label(cropping_frame, text=description, font=("Helvetica", 8), bg="#ffffff")
            desc_label.grid(row=idx, column=2, sticky='w', padx=5, pady=2)

        # Area Threshold Section
        area_threshold_frame = tk.LabelFrame(control_frame, text="Area Threshold", font=self.font_subtitle,
                                             bg="#ffffff", padx=10, pady=10)
        area_threshold_frame.grid(row=3, column=0, columnspan=3, pady=5, sticky='we')  # Spanning all columns

        area_threshold_label = tk.Label(area_threshold_frame, text="Area Threshold:", font=self.font_labels, bg="#ffffff")
        area_threshold_label.grid(row=0, column=0, sticky='w', padx=5, pady=2)

        area_threshold_entry = tk.Entry(area_threshold_frame, font=self.font_entries)
        area_threshold_entry.grid(row=0, column=1, sticky='w', padx=5, pady=2)
        default_val = self.config.get("DEFAULT", "area_threshold", fallback="2")
        area_threshold_entry.insert(0, default_val)
        self.entries["area_threshold"] = area_threshold_entry

        area_threshold_desc_label = tk.Label(area_threshold_frame, text="Minimum area threshold for object detection.", font=("Helvetica", 8), bg="#ffffff")
        area_threshold_desc_label.grid(row=0, column=2, sticky='w', padx=5, pady=2)
        ...
        adaptive_frame = tk.LabelFrame(control_frame, text="Adaptive Thresholding", font=self.font_subtitle,
                                    bg="#ffffff", padx=10, pady=10)
        adaptive_frame.grid(row=4, column=0, columnspan=3, pady=5, sticky='we')  # Spanning all columns

        adaptive_settings = [
            ("Enable Adaptive Thresholding", "adaptive_threshold", "Enable or disable adaptive thresholding."),
            ("Adaptive Window Size", "adaptive_window_size", "Size of the window for adaptive thresholding."),
            ("Adaptive C Constant", "adaptive_C", "Constant subtracted from the mean in adaptive thresholding."),
            ("Kernel Size (e.g., (5, 5))", "kernel_size", "Size of the kernel for morphological operations.")
        ]

        for idx, (label_text, key, description) in enumerate(adaptive_settings):
            ...
            desc_label = tk.Label(adaptive_frame, text=description, font=("Helvetica", 8), bg="#ffffff")
            desc_label.grid(row=idx, column=2, sticky='w', padx=5, pady=2)

        # Color Threshold Section
        color_threshold_frame = tk.LabelFrame(control_frame, text="Color Threshold", font=self.font_subtitle,
                                            bg="#ffffff", padx=10, pady=10)
        color_threshold_frame.grid(row=5, column=0, columnspan=3, pady=5, sticky='we')  # Spanning all columns

        color_threshold_settings = [
            ("Color Threshold (RGB):", "color_threshold", "Threshold for color-based filtering.")
        ]

        for idx, (label_text, key, description) in enumerate(color_threshold_settings):
            ...
            desc_label = tk.Label(color_threshold_frame, text=description, font=("Helvetica", 8), bg="#ffffff")
            desc_label.grid(row=idx, column=2, sticky='w', padx=5, pady=2)

        # RGB Filtering Section
        rgb_filter_frame = tk.LabelFrame(control_frame, text="RGB Filtering", font=self.font_subtitle,
                                        bg="#ffffff", padx=10, pady=10)
        rgb_filter_frame.grid(row=6, column=0, columnspan=3, pady=5, sticky='we')  # Spanning all columns

        rgb_filter_settings = [
            ("Enable RGB Filter", "filter_rgb", "Enable or disable RGB filtering."),
            ("R Threshold", "r_threshold", "Threshold for the red channel."),
            ("G Threshold", "g_threshold", "Threshold for the green channel."),
            ("B Threshold", "b_threshold", "Threshold for the blue channel.")
        ]

        for idx, (label_text, key, description) in enumerate(rgb_filter_settings):
            if key == "filter_rgb":
            ...
            desc_label = tk.Label(rgb_filter_frame, text=description, font=("Helvetica", 8), bg="#ffffff")
            desc_label.grid(row=idx, column=2, sticky='w', padx=5, pady=2)

        # Create a frame for action buttons at the bottom of control_frame
        buttons_frame = tk.Frame(control_frame, bg="#ffffff")
        buttons_frame.grid(row=7, column=0, columnspan=3, pady=20, sticky='w')  # Aligned to left
